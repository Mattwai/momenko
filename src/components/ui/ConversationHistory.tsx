...
import { View, StyleSheet, ScrollView, TouchableOpacity, Alert, Share, Vibration } from 'react-native';
import { Text, Surface, Searchbar, IconButton, Menu, Divider, Chip, Button, Portal, Modal } from 'react-native-paper';
...
const ConversationHistory: React.FC<ConversationHistoryProps> = ({
  ...
  const handleExportSession = useCallback(async (session: ConversationSession) => {
    try {
      if (!onExportSession) {
        Alert.alert('Export Not Available', 'Export functionality is not configured.');
        return;
      }
      ...
      if (!validation.isAppropriate) {
        Alert.alert(
          ...
          [
            ...
            { text: 'Export Anyway', onPress: () => onExportSession(session) }
          ]
        );
        return;
      }
  ...
  const filteredSessions = useMemo(() => {
    let filtered = sessions.filter(session => {
      ...
      if (searchQuery) {
        ...
        const matches = session.summary?.toLowerCase().includes(searchLower) ||
      ...
      if (filterTags.length > 0) {
        const hasFilterTag = session.tags?.some(tag => filterTags.includes(tag));
  ...
  const getCulturalIcon = (culturalGroup: string) => {
    switch (culturalGroup) {
      case 'maori': return 'spiral';
      case 'chinese': return 'yin-yang';
      case 'western': return 'account';
    ...
    return (
      ...
      >
        ...
        >
            ...
            <View style={styles.sessionHeaderContent}>
              <View style={styles.sessionTitleRow}>
                ...
                <View style={styles.sessionInfo}>
                  ...
                  >
                    {format(session.startTime, 'p', { locale: getDateLocale() })} • {duration} • {session.messages.length} messages
                  </Text>
                </View>
              </View>
          ...
          {isExpanded && (
            <Animatable.View animation="slideInDown" style={styles.expandedContent}>
        ...
        <View style={styles.filtersRow}>
          <TouchableOpacity
            style={[
              styles.filterButton,
              {
                backgroundColor: showFilters ? colors.primary : (isHighContrast ? '#FFFFFF' : '#F3F4F6'),
            ...
            <View style={styles.languageFilters}>
              {['all', 'en', 'mi', 'zh'].map((lang) => (
                <Chip
                  ...
                  onPress={() => setSelectedLanguage(lang as typeof selectedLanguage)}
                  style={[
                    styles.languageChip,
                    selectedLanguage === lang && { backgroundColor: colors.primary }
                  ]}
                  textStyle={{
                    fontSize: textSizes.caption,
                    color: selectedLanguage === lang ? '#FFFFFF' : colors.primary
                  }}
                >
                ...
                <View style={styles.tagChips}>
                  {allTags.map((tag) => (
                    <Chip
                      ...
                      style={[
                        styles.tagChip,
                        filterTags.includes(tag) && { backgroundColor: colors.accent }
                      ]}
                      textStyle={{
                        fontSize: textSizes.caption - 2,
                        color: filterTags.includes(tag) ? '#FFFFFF' : colors.accent
                      }}
                    >
                      {tag}
                    </Chip>
      ...
      >
        {filteredSessions.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            >
              {searchQuery || selectedLanguage !== 'all' || filterTags.length > 0
        ...
        >
          {selectedSession && (
            <View>
              ...
              >
                {format(selectedSession.startTime, 'PPPp', { locale: getDateLocale() })}
              </Text>
