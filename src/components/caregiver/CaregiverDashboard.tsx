...
import { Card, Button, FAB, Portal, Dialog, TextInput } from 'react-native-paper';
import Icon from 'react-native-vector-icons/MaterialIcons';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { format, subDays, isToday } from 'date-fns';
import { supabase } from '../../services/supabase';
import { notificationService, backgroundScheduler } from '../../services/notifications';
import {
  CaregiverAlert,
  WellnessIndicator,
  CheckInSchedule,
...
const CaregiverDashboard: React.FC = () => {
  ...
  const loadPatients = async () => {
    ...
    const patientsData: PatientOverview[] = data?.map(user => {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Cultural-Aware Care Management</Text>
      </View>
      ...
      >
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {patients.map(patient => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.patientCard,
                    selectedPatient === patient.id && styles.selectedPatientCard,
                  ]}
                ...
                >
                  ...
                  <View style={styles.patientStats}>
                    <View style={[styles.wellnessScore, { backgroundColor: getWellnessColor(patient.currentWellnessScore) }]}>
                      <Text style={styles.wellnessScoreText}>{patient.currentWellnessScore}</Text>
                    </View>
        ...
        {selectedPatientData && (
          <>
            ...
            <Card style={styles.card}>
              <Card.Content>
                ...
                <View style={styles.culturalBadge}>
                  ...
                  />
                  <Text style={[styles.culturalText, { color: getCulturalColor(selectedPatientData.culturalGroup) }]}>
                    {selectedPatientData.culturalGroup.charAt(0).toUpperCase() + selectedPatientData.culturalGroup.slice(1)} Cultural Profile
                  </Text>
                </View>
                <View style={styles.wellnessGrid}>
                  <View style={styles.wellnessItem}>
                    <Text style={styles.wellnessLabel}>Wellness Score</Text>
                    <Text style={[styles.wellnessValue, { color: getWellnessColor(selectedPatientData.currentWellnessScore) }]}>
                      {selectedPatientData.currentWellnessScore}%
                    </Text>
                  </View>
                  <View style={styles.wellnessItem}>
                    <Text style={styles.wellnessLabel}>Check-in Streak</Text>
                    <Text style={styles.wellnessValue}>{selectedPatientData.checkInStreak} days</Text>
                  </View>
                  <View style={styles.wellnessItem}>
                    ...
                    <Text style={styles.wellnessValue}>
                      {selectedPatientData.lastCheckIn 
                        ? isToday(new Date(selectedPatientData.lastCheckIn)) 
                          ? 'Today' 
                          : format(new Date(selectedPatientData.lastCheckIn), 'MMM d')
                        : 'Never'
                      }
                    </Text>
                  </View>
                ...
                {conversationSummaries.slice(0, 3).map(summary => (
                  <View key={summary.id} style={styles.conversationItem}>
                    <View style={styles.conversationHeader}>
                      ...
                      <View style={[styles.qualityBadge, { backgroundColor: getWellnessColor(summary.quality === 'excellent' ? 100 : summary.quality === 'good' ? 75 : summary.quality === 'fair' ? 50 : 25) }]}>
                        <Text style={styles.qualityText}>{summary.quality}</Text>
                      </View>
                    </View>
                    <Text style={styles.conversationDuration}>Duration: {summary.duration} minutes</Text>
            ...
            <Card style={styles.card}>
              <Card.Content>
                <Text style={styles.cardTitle}>Cultural Education Resources</Text>
                <Text style={styles.resourceSubtitle}>
                  Learn about {selectedPatientData.culturalGroup} cultural practices
                </Text>
                <View style={styles.resourceGrid}>
                  ...
                  <TouchableOpacity style={styles.resourceItem}>
                    ...
                    <Text style={styles.resourceText}>Spiritual Considerations</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.resourceItem}>
            ...
            {selectedAlert?.suggestedActions && (
              <View style={styles.suggestedActions}>
                ...
                {selectedAlert.suggestedActions.map((action, index) => (
                  <Text key={index} style={styles.suggestedAction}>• {action}</Text>
                ))}
              </View>
            )}
            <TextInput
