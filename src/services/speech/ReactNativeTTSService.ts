...
import { PreferredLanguage, AzureVoiceConfig } from '../../types';
...
export class ReactNativeTTSService {
  ...
  private getCulturalSpeechOptions(language: PreferredLanguage): Partial<SpeechOptions> {
  ...
  async synthesizeSpeech(
    ...
    try {
      ...
      console.log(`Starting TTS for language ${this.currentLanguage}:`, adjustedText.substring(0, 50) + '...');
  ...
  static async getVoicesForLanguage(language: PreferredLanguage): Promise<Speech.Voice[]> {
    try {
      const allVoices = await Speech.getAvailableVoicesAsync();
      const languageCode = new ReactNativeTTSService({ language }).getLanguageCode(language);
  ...
  async speakWithOptions(
    ...
    try {
      ...
      const adjustedText = this.addCulturalPauses(text, this.currentLanguage);
      const defaultOptions = this.getCulturalSpeechOptions(this.currentLanguage);
